...
7. 인터페이스를 새로 추가하거나 확장했다면 `docs/ARCHITECTURE.md` 의 확장 포인트 표를 같은 PR 에서 갱신한다. 게임 규칙 수치를 바꿨다면 `docs/GDD.md` 를 갱신한다.
8. 요청 범위 밖의 리팩터링, 의존성 추가/교체, 설정 파일 변경은 하지 않는다. 필요하다면 PR 본문에 제안만 적는다.
9. 막히면 임의로 우회하지 말고 PR 또는 이슈에 실패 로그와 함께 상황을 보고한다.

## 4.11 백로그 제안 모드 (Semi-auto Loop)

### 4.11.1 진입 조건

세션 시작 시 PROGRESS.md 를 읽고 미완료 태스크("- [ ]" 로 시작하는 항목) 개수를 센다.

- 미완료 태스크가 1개 이상이면 정상 루프 모드로 진행한다.
- 미완료 태스크가 0개이면 백로그 제안 모드로 진입한다.

### 4.11.2 백로그 제안 모드 절차

1. CONTEXT.md, docs/GDD.md, docs/ROADMAP.md 를 순서대로 다시 읽는다.

2. 현재 게임의 정체성(물리 기반 머지 로그라이트)과 도파민 설계 4요소, 차별화 메커니즘 3종을 재확인한다.

3. 다음 5개 태스크를 제안한다. 각 태스크는 다음 조건을 만족해야 한다.
...
4. docs/PROPOSED_TASKS.md 파일을 새로 만들고 제안 5개를 다음 형식으로 작성한다.
...
5. feat/proposal-batch-1 브랜치를 만들어 커밋하고 PR 을 생성한다. 배치 번호는 1부터 시작하여 매 배치마다 1씩 증가한다.

6. PR 본문에 다음 문구를 포함한다. "이 PR 은 코드 변경이 아니라 다음 배치의 태스크 제안입니다. 채택할 항목을 알려주세요."

7. 이 세션에서는 코드를 변경하지 않는다. PROGRESS.md 도 수정하지 않는다. npm 검증 절차도 생략한다.

### 4.11.3 사용자 승인 처리

사용자가 제안을 채택하기로 결정하면 다음 세션에서 사용자가 채택할 번호를 알려준다.

예: "docs/PROPOSED_TASKS.md 의 제안 중 1번, 3번, 5번을 채택한다."

에이전트는 이 지시에 따라 다음을 수행한다.

1. PROGRESS.md 백로그 끝에 채택된 태스크를 새 번호로 추가한다. 기존 번호는 재사용하지 않는다.

2. Active Next Action 을 채택된 첫 태스크로 갱신한다.

3. docs/PROPOSED_TASKS.md 는 그대로 둔다(이력 보존).

4. feat/proposal-accept-1 브랜치로 PR 을 생성한다.

5. 이 세션도 하나의 루프로 계산한다.
