...
| ----------------------- | ------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | ----------------------- |
| 머지 카드 / 리스크 카드 | `src/core/interfaces/IMergeCard.ts` (`MergeCard`, `RiskCard`, `IMergeCardProvider`, `MergeCardContext`) | `src/systems/cards/BasicMergeCardProvider.ts` (보상 3종 + 리스크 3종, 시드 결정적 드로)                                             | `createApp.ts` 에서 `CardSlowMotionSelector` 에 주입                                      | Task 1.4                |
| 슬로우모션 선택         | `src/core/interfaces/ISlowMotionSelector.ts` (`offerCards?` 필드 추가 Task 2.13)                        | `src/systems/CardSlowMotionSelector.ts` (티어 2 이상 또는 최대 티어 소멸 시 발동, 선택 창 종료 시 패 폐기, 라운드 보상 패 기억)    | `new Game({ slowMotionSelector })` + `Game.openRewardChoice` (라운드 보상 선택)           | Task 2.13 완료          |
...
| 특수 공                 | `src/core/interfaces/ISpecialBall.ts`                                                                   | `src/systems/special/BombBallBehavior.ts`(`blastVictims` + `blastScore`) + `SpecialBallRegistry`                       | `Game` 의 specialBalls, `createApp` 에서 등록. 폭발 점수는 `ball:detonated.scoreGained` 으로 전달 | Task 2.14 완료 |
| 라운드                  | `src/core/interfaces/IRoundSystem.ts` (`isDropBudgetExhausted` 필드 추가 Task 2.2, `RoundHudState` + `getHudState?` Task 2.12) | `src/systems/BasicRoundSystem.ts` + `src/systems/RoundRunner.ts`(이벤트 구독·보상 지급·`getHudState`) | `createApp.ts` 에서 `RoundRunner` 가 `EventBus` 구독 + `game.openRewardChoice`(선택)/`game.applyRewardCard`(폴백), `src/app/composeRoundSnapshot.ts` 가 `getHudState()` 를 `GameSnapshot.round` 으로 합성해 HUD 표시(렌더 콜백, Task 2.15) | Task 2.15 완료 |
...
| 스폰 하한 페널티        | `src/core/interfaces/IMergeCard.ts` (`MergeCardContext.raiseSpawnTierFloor?`, Task 2.16) | `src/core/ball/SpawnTierPenalty.ts` — 하한 티어 + 남은 발급 수(카드 구현은 `src/systems/cards/SpawnLargerBallsCard.ts`, Task 2.18), 중첩 시 각각 최댓값, 만료/리셋 시 해제 | `Game` 이 상태 객체를 소유하고 `buildCardContext` 가 위임, 신규 발급 roll 에만 적용(Task 2.17 완료). `GameSnapshot.spawnPenalty`(읽기 전용 복사, 비활성 시 필드 없음)를 `src/render/SpawnPenaltyHudRenderer.ts` 가 NEXT 아래에 표시(Task 2.19 완료) | Task 2.19 완료 |
