...
| 스폰 하한 페널티        | `src/core/interfaces/IMergeCard.ts` (`MergeCardContext.raiseSpawnTierFloor?`, Task 2.16) | `src/core/ball/SpawnTierPenalty.ts` — 하한 티어 + 남은 발급 수, 중첩 시 각각 최댓값, 만료/리셋 시 해제 | `Game.buildCardContext` 가 위임하고 신규 발급 티어에만 적용(Task 2.17), `GameSnapshot.spawnPenalty` 로 HUD 노출(Task 2.19) | Task 2.17~2.19 |
