...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- |
| 머지 카드 / 리스크 카드 | `src/core/interfaces/IMergeCard.ts` (`MergeCard`, `RiskCard`, `IMergeCardProvider`, `MergeCardContext` + `raiseSpawnTierFloor?` Task 2.16) | `src/systems/cards/BasicMergeCardProvider.ts` (보상 3종 + 리스크 3종 — HEAVY LOAD `SpawnLargerBallsCard` 포함, 시드 결정적 드로)    | `createApp.ts` 에서 `CardSlowMotionSelector` 에 주입. `raiseSpawnTierFloor` 은 `Game` 이 `SpawnTierPenalty` 로 구현해 `GameSnapshot.spawnPenalty`(Task 2.19) 로 노출            | Task 1.4 · 2.16~2.19 완료 |
...
| 라운드                  | `src/core/interfaces/IRoundSystem.ts` (`isDropBudgetExhausted` 필드 추가 Task 2.2, `RoundHudState` + `getHudState?` Task 2.12)             | `src/systems/BasicRoundSystem.ts` + `src/systems/RoundRunner.ts`(이벤트 구독·보상 지급·`getHudState`)                               | `createApp.ts` 에서 `RoundRunner` 가 `EventBus` 구독 + `game.openRewardChoice`(선택)/`game.applyRewardCard`(폴백), `getHudState()` 를 `GameSnapshot.round` 으로 합성해 HUD 표시 | Task 2.13 완료            |
