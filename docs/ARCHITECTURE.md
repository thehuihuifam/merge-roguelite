...
| 슬로우모션 선택         | `src/core/interfaces/ISlowMotionSelector.ts`                                                            | `src/systems/CardSlowMotionSelector.ts` (티어 2 이상 또는 최대 티어 소멸 시 발동, 선택 창 종료 시 패 폐기)                          | `new Game({ slowMotionSelector })`                                                        | Task 1.4                |
...
| 라운드                  | `src/core/interfaces/IRoundSystem.ts` (`isDropBudgetExhausted` 필드 추가 Task 2.2, `RoundHudState` + `getHudState?` Task 2.12) | `src/systems/BasicRoundSystem.ts` + `src/systems/RoundRunner.ts`(이벤트 구독·보상 지급·`getHudState`) | `createApp.ts` 에서 `RoundRunner` 가 `EventBus` 구독 + `game.applyRewardCard`, `getHudState()` 를 `GameSnapshot.round` 으로 합성해 HUD 표시 | Task 2.13 (보상 선택지) |
