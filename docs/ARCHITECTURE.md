...
| ----------------------- | ------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | ------------------------------- |
| 머지 카드 / 리스크 카드 | `src/core/interfaces/IMergeCard.ts` (`MergeCard`, `RiskCard`, `IMergeCardProvider`, `MergeCardContext`) | `src/systems/cards/BasicMergeCardProvider.ts` (보상 3종 + 리스크 2종, 시드 결정적 드로) | `ISlowMotionSelector` 구현체가 사용 (주입은 Task 1.3)             | Task 1.3, 1.4                   |
| 슬로우모션 선택         | `src/core/interfaces/ISlowMotionSelector.ts`                                                            | `src/systems/NoopSlowMotionSelector.ts`                                                 | `new Game({ slowMotionSelector })`                                | Task 1.3, 1.4                   |
| 근접 실패 연출          | `src/core/interfaces/INearMissEffect.ts`                                                                | `src/systems/NoopNearMissEffect.ts`                                                     | `new Game({ nearMissEffect })`                                    | Task 2.1                        |
| 점수 수정자 (배율 카드) | `src/core/interfaces/IScoreModifier.ts`                                                                 | 없음. `MergeCardContext.pushScoreMultiplier` 가 임시 수정자를 만든다                    | `new Game({ scoreModifiers })` 또는 `ScoreCalculator.addModifier` | Task 2.7                        |
| 특수 공                 | `src/core/interfaces/ISpecialBall.ts`                                                                   | 없음                                                                                    | 향후 `MergeRules.canMerge` 앞단에서 조회                          | Task 2.3                        |
| 라운드                  | `src/core/interfaces/IRoundSystem.ts`                                                                   | 없음                                                                                    | `EventBus` 구독 (`ball:dropped`, `score:changed`)                 | Task 2.2                        |
| 세이브                  | `src/core/interfaces/ISaveSystem.ts`                                                                    | 없음                                                                                    | `createApp.ts` 에서 `initialBest` 주입 + `run:over` 구독          | Task 2.4                        |
| 사운드                  | `src/core/interfaces/IAudioSystem.ts`                                                                   | 없음                                                                                    | `EventBus` 구독                                                   | Task 2.6                        |
| 파티클                  | `src/core/interfaces/IParticleSystem.ts`                                                                | 없음                                                                                    | `EventBus` 구독 + `CanvasRenderer` 에서 `render(ctx)`             | Task 2.5                        |
| 위험선 규칙             | `src/core/danger/OverflowDetector.ts` (옵션 객체)                                                       | 기본 상수                                                                               | `new Game({ overflowDetector })`                                  | 리스크 카드 `raise_danger_line` |
