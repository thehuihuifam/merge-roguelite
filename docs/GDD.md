...
**승리:** v0.1.0 은 무한 모드다. 명시적 승리는 없고 점수와 세션 베스트가 성과다. 2048 두 개의 합체(보드 정리 + 10,000점)가 사실상의 마일스톤 역할을 한다. v0.2.0 의 `IRoundSystem` 이 라운드 목표 점수를 도입하면 "라운드 클리어" 가 중간 승리 조건이 되고, v1.0.0 에서 최종 라운드 클리어를 런 승리로 정의한다(`docs/ROADMAP.md`).
